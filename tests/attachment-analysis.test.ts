...
describe("attachment metadata analysis", () => {
  it.each([
    ...
    expect(assessAttachment({ filename, mimeType })).toMatchObject({ kind, points });
  });
  ...
  it("uses one score per attachment and caps the total contribution", () => {
    const result = analyzeAttachments([
      ...
      { filename: "update.scr", mimeType: "application/octet-stream" },
    ]);
    expect(result.points).toBe(45);
  });

  it("keeps ordinary attachment names at zero while explaining they were not scanned", () => {
    const result = analyzeAttachments([{ filename: "report.pdf", mimeType: "application/pdf" }]);
    expect(result.points).toBe(0);
    expect(result.evidence[0].title).toBe("Attachments were not scanned");
  });

  it("validates bounded attachment metadata", () => {
    expect(isAttachmentMetadataList([{ filename: "report.pdf", mimeType: "application/pdf" }])).toBe(true);
    expect(isAttachmentMetadataList([{ filename: "", mimeType: "application/pdf" }])).toBe(false);
